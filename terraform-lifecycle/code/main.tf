...
resource "aws_s3_bucket" "critical_data" {
  bucket = "my-critical-production-data-${var.environment}-${data.aws_region.current.name}-42000"
...
resource "aws_s3_bucket" "regional_validation" {
  bucket = "validated-region-bucket-${var.environment}-${data.aws_region.current.name}-19051017420"
...
resource "aws_s3_bucket" "compliance_bucket" {
  bucket = "compliance-bucket-${var.environment}-${data.aws_region.current.name}-190510174204"
...
resource "aws_s3_bucket" "app_buckets" {
  ...
  bucket = "${each.value}-${var.environment}-190510174201"
